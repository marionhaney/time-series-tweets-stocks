...
For tweets, we decided to break them down into a bag of words representation. 
Each word is given a sentiment score of 1 if it is positive, -1 if it is 
negative, and 0 otherwise (neutral). These sentiment scores are summed into a 
sentiment score for the tweet. Sentiment scores are averaged into a daily 
tweet sentiment score for the day.
...
data because of the high volatility. We first ran auto.arima on the 
returns^2 = (diff of the log of adjusted closing price)^2 to get the order of 
the GARCH model. Based on this order, we decided to fit a GARCH(1, 1) model to 
returns (diff of the log of the adjusted closing price). 
...
returns. This means that we can predict the sentiment with an adjusted r2 of 
10%, these features account for 10% of the variability of daily sentiment. Past 
returns and past daily sentiment granger causes current daily sentiment (that 
means that past returns and past daily sentiment can be used to predict the 
daily sentiment for the current day). What we would have wanted is to use past
returns and past daily sentiment to predict current returns. 
